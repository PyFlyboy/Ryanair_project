...
from src.utilities import custom_logger as tl
import logging

class Util(object):


    log = tl.custom_logger(logging.INFO)
    ...
    def generate_radom_string(self, n=2, capital=False):
        """
        ...
        if capital == True:
            ran_string = ran.upper()
        else:
            ran_string = ran.lower()
        return ran_string

    def generate_user_email(self):
        gen_random_string = self.generate_radom_string()
        user_email = 'testwebchilitest+' + gen_random_string + '@gmail.com'
        return user_email

    def generate_regon(self):
        ran = ''.join(random.choices(string.digits, k=9))
        return ran

    def generat_income_value(self):
